...
<header class="site-header <?php echo is_front_page() ? 'home-header' : ''; ?>">
    ...
    <div class="nav-shell">
        ...
        <button class="icon-button mobile-menu-toggle" type="button" aria-controls="mobile-menu" aria-haspopup="dialog" aria-expanded="false" aria-label="Abrir menú"><?php echo tienda_icon('menu'); ?></button>
        <nav id="primary-nav" aria-label="Menú principal"><?php wp_nav_menu( array( 'theme_location' => 'primary', 'container' => false, 'items_wrap' => '<ul class="%2$s">%3$s</ul>', 'menu_class' => 'menu', 'fallback_cb' => 'gamilea_primary_menu_fallback' ) ); ?></nav>
