(() => {
  ...
  if (menu && mobileMenu) {
    ...
    mobileMenu.addEventListener('close', () => {
      ...
      if (window.matchMedia('(max-width: 1100px)').matches) menu.focus();
    });
  ...
  let favorites = [];
  function favoriteURL(value) {
    if (typeof value !== 'string' || !value.trim()) return '';
    try { const url = new URL(value, location.origin); return ['http:', 'https:'].includes(url.protocol) ? url.href : ''; } catch { return ''; }
  }
  function loadFavorites() {
    try {
      const saved = JSON.parse(localStorage.getItem('tienda-favorites') || '[]');
      if (!Array.isArray(saved)) return;
      favorites = saved.filter(item => item && typeof item.id === 'string' && typeof item.name === 'string' && favoriteURL(item.url))
        .map(item => ({ ...item, url: favoriteURL(item.url), image: favoriteURL(item.image) }));
    } catch {}
  }
  loadFavorites();
  ...
  function syncFavorites(persist = true) { document.querySelectorAll('.favorite-toggle').forEach(button => { const saved=favorites.some(p=>p.id===button.dataset.id); button.setAttribute('aria-pressed',String(saved)); }); if (!persist) return true; try { localStorage.setItem('tienda-favorites', JSON.stringify(favorites)); return true; } catch { return false; } }
  document.addEventListener('click', e => { const button=e.target.closest('.favorite-toggle'); if(!button) return; loadFavorites(); const id=button.dataset.id; const exists=favorites.some(p=>p.id===id); if(exists) favorites=favorites.filter(p=>p.id!==id); else { const card=button.closest('.product-card'); favorites.push({id,name:card.dataset.productName || card.querySelector('h3').textContent,url:card.querySelector('h3 a').href,image:card.querySelector('.product-photo img')?.src || '',sprite:card.querySelector('.product-sprite')?.className || ''}); } const persisted = syncFavorites(); announce(persisted ? (exists ? 'Producto eliminado de favoritos' : 'Producto guardado en favoritos') : 'No se pudo guardar el cambio en este navegador.'); });
  const dialog=document.getElementById('favorites-dialog');
  function renderFavorites() { const content=document.getElementById('favorites-content'); content.replaceChildren(); if(!favorites.length) {const p=document.createElement('p');p.textContent='Todavía no tienes favoritos. Toca el corazón de un producto para guardarlo.';content.append(p);} favorites.forEach(product=>{const row=document.createElement('div');row.className='favorite-item';const a=document.createElement('a');a.href=product.url;const img=document.createElement('img');if(product.image) img.src=product.image; else img.hidden=true;img.alt='';const title=document.createElement('span');title.textContent=product.name;if(typeof product.sprite==='string' && /^product-sprite sprite-[0-4]$/.test(product.sprite)){const crop=document.createElement('span');crop.className=product.sprite;crop.append(img);a.append(crop,title);}else{a.append(img,title);}const remove=document.createElement('button');remove.textContent='Quitar';remove.addEventListener('click',()=>{loadFavorites();favorites=favorites.filter(p=>p.id!==product.id);if(!syncFavorites()) announce('No se pudo guardar el cambio en este navegador.');renderFavorites();});row.append(a,remove);content.append(row);}); }
  document.querySelector('.open-favorites')?.addEventListener('click',()=>{loadFavorites();syncFavorites(false);renderFavorites();dialog.showModal();});
  document.querySelector('.close-favorites')?.addEventListener('click',()=>dialog.close());
  dialog?.addEventListener('click',e=>{if(e.target===dialog){const rect=dialog.getBoundingClientRect();if(e.clientX<rect.left||e.clientX>rect.right||e.clientY<rect.top||e.clientY>rect.bottom)dialog.close();}});
  window.addEventListener('storage', event => { if (event.key === 'tienda-favorites' || event.key === null) { loadFavorites(); syncFavorites(false); if (dialog?.open) renderFavorites(); } });
  syncFavorites(false);
